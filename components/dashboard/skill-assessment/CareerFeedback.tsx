...
import { CheckCircle, AlertCircle, Download, Share2, RefreshCw, Trophy, Code, Monitor, Database } from 'lucide-react';
import { AssessmentData } from '@/types/resume';

const CareerFeedback: React.FC<{
  assessmentData: AssessmentData;
  onRetakeAssessment: () => void;
}> = ({ assessmentData, onRetakeAssessment }) => {
  const [activeTab, setActiveTab] = useState<'technical' | 'soft'>('technical');

  const { results, learning_recommendations, skills } = assessmentData;
  ...
  const getIconForRole = (roleName: string) => {
    ...
    } else if (role.includes('backend') || role.includes('api')) {
      return <Database className="text-gray-600" size={20} />;
    } else if (role.includes('full stack') || role.includes('developer')) {
        ...
        <div className='bg-white p-6 rounded-lg shadow-sm'>
          ...
          <div className="space-y-3 mb-8">
            {career_suggestions.suggestions.map((suggestion, index) => {
              const matchPercent = parseFloat(suggestion.match_percent);
              return (
                <div key={index} className={`flex items-center justify-between p-3 rounded-lg ${matchPercent >= 90 ? 'bg-green-50' :
                    matchPercent >= 80 ? 'bg-blue-50' :
                      'bg-orange-50'
                  }`}>
                  <div className="flex items-center gap-3">
                    {getIconForRole(suggestion.role_name)}
                    <span className="font-medium">{suggestion.role_name}</span>
                  </div>
                  <span className={`px-2 py-1 rounded text-sm font-medium ${matchPercent >= 90 ? 'bg-green-200 text-green-800' :
                      matchPercent >= 80 ? 'bg-blue-200 text-blue-800' :
